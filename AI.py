...
def render_calendar():
    ...
    for idx, w in enumerate(["一", "二", "三", "四", "五", "六", "日"]):
        week_cols[idx].markdown(f"**{w}**")

    # 日历格子
    cal = calendar.monthcalendar(year, month)
    for row in cal:
        day_cols = st.columns(7)
        for idx, day in enumerate(row):
            if day == 0:
                day_cols[idx].empty()
                continue
            # 日期状态：过去/今日/未来
            ...
            # 按钮样式
            btn_style = "opacity: 0.5;" if is_past else ""
            if is_today:
                btn_style += "background-color: #22c55e; color: white;"

            # 日期按钮
            with day_cols[idx]:
                if st.button(str(day), key=f"day_{day}", on_click=lambda d=day: st.session_state.update({
                    "selected_day": d,
                    "selected_date_str": f"{year}-{month:02d}-{d:02d}"
                })):
                    st.rerun()
                # 过去日期变灰
                if is_past:
                    st.markdown(f"<div style='opacity: 0.4; margin-top: -30px;'>{day}</div>", unsafe_allow_html=True)

    # 日程编辑
    st.markdown("---")
    st.markdown(f"### 📝 {selected_date_str} 日程")
    current_schedule = st.session_state.schedule.get(selected_date_str, "")
    new_schedule = st.text_area("", current_schedule, height=100, label_visibility="collapsed",
                                placeholder="输入当日日程...")
    if new_schedule != current_schedule:
        st.session_state.schedule[selected_date_str] = new_schedule
        save_all_data()
