...
def generate_daily_diary(chat_history, today_date):
    """生成每日日记"""
    diary_prompt = f"""
    今天是 {today_date}。请根据今天的聊天记录，写一篇简短的日记，记录我们的对话和你的感受。
    聊天记录：{chat_history}
    日记要求：
    1.  以第一人称“我”（梁叙柏）的视角来写。
    2.  内容要温暖、真诚，记录下今天的关键对话和你的思考。
    3.  字数控制在150-200字左右。
    """
    # 调用AI生成日记
...
def init_session_state():
    ...
    if "last_diary_time" not in st.session_state:
        st.session_state.last_diary_time = None

    # ========== 新增：日记存储相关状态 ==========
    if "diaries" not in st.session_state:
        st.session_state.diaries = {}  # key: 日期字符串 "YYYY-MM-DD", value: {"my": "...", "his": "...", "my_image": None, "his_image": None}
    if "my_diary_image" not in st.session_state:
        st.session_state.my_diary_image = None
    if "show_his_diary" not in st.session_state:
        st.session_state.show_his_diary = False
    # ============================================
    ...
    if 6 <= now.hour < 7 and (
            st.session_state.last_greeting_time is None or st.session_state.last_greeting_time.date() != now.date()):
    ...
    # 检查是否需要发送晚安 (0:00-0:30)
        ...
        st.session_state.messages.append({"role": "assistant", "content": greeting})
        st.session_state.last_greeting_time = now
    # 检查是否需要生成日记 (21:00-21:30)
    elif 21 <= now.hour < 22 and (
            st.session_state.last_diary_time is None or st.session_state.last_diary_time.date() != now.date()):
        # 提取今天的聊天记录
        today_chat = [msg["content"] for msg in st.session_state.messages if
                      msg["role"] == "user" or msg["role"] == "assistant"]
        diary = generate_daily_diary(today_chat, today_date_str)
        st.session_state.messages.append({"role": "assistant", "content": f"📖 今日日记：\n{diary}"})
        # 自动保存到日记列表
        if today_date_str not in st.session_state.diaries:
            st.session_state.diaries[today_date_str] = {"my": "", "his": "", "my_image": None, "his_image": None}
        st.session_state.diaries[today_date_str]["his"] = diary
        st.session_state.last_diary_time = now
...
def get_ai_response(prompt, is_diary=False):
    ...
    data = {"model": MODEL_ENDPOINT, "messages": messages, "temperature": 0.6, "max_tokens": 50}
    try:
    ...
    with st.sidebar:
        ...
        for i in range(3):
            with st.container(border=True, height=130):
                st.text_input(f"标题", value=st.session_state.countdowns[i]['name'], key=f"cd_name_{i}",
                              label_visibility="collapsed", placeholder=f"目标 {i + 1}")
                selected_date = st.date_input(f"日期", value=datetime.strptime(st.session_state.countdowns[i]['date'],
                                                                               "%Y-%m-%d") if
                st.session_state.countdowns[i]['date'] else None, key=f"cd_date_{i}", label_visibility="collapsed")
        ...
        # 图片上传
        uploaded_file = st.file_uploader("上传图片（可选）", type=["png", "jpg", "jpeg"], label_visibility="collapsed")
        if uploaded_file is not None:
            st.session_state.my_diary_image = uploaded_file

        # 保存日记按钮
        if st.button("保存我的日记"):
            if st.session_state.diary:
                if today_date_str not in st.session_state.diaries:
                    st.session_state.diaries[today_date_str] = {"my": "", "his": "", "my_image": None,
                                                                "his_image": None}
                st.session_state.diaries[today_date_str]["my"] = st.session_state.diary
                st.session_state.diaries[today_date_str]["my_image"] = st.session_state.my_diary_image
                st.success("日记已保存！")
        ...
        # ========== 新增：Ta的DIARY 模块 ==========
        st.subheader("💌 Ta的DIARY")
        if st.button("看看他的日记"):
            st.session_state.show_his_diary = not st.session_state.show_his_diary

        if st.session_state.show_his_diary and today_date_str in st.session_state.diaries:
            his_diary = st.session_state.diaries[today_date_str]["his"]
            his_image = st.session_state.diaries[today_date_str]["his_image"]
            if his_diary:
                st.write(his_diary)
                if his_image:
                    st.image(his_image, caption="他的图片")
            else:
                st.info("他今天还没写日记哦~")
        ...
        # 日期选择器
        selected_date = st.date_input("选择日期", datetime.now(), label_visibility="collapsed")
        selected_date_str = selected_date.strftime("%Y-%m-%d")

        # 显示选中日期的日记
        if selected_date_str in st.session_state.diaries:
            diary_entry = st.session_state.diaries[selected_date_str]
            st.write("**你的日记：**")
            st.write(diary_entry["my"])
            if diary_entry["my_image"]:
                st.image(diary_entry["my_image"], caption="你的图片")

            st.write("**他的日记：**")
            st.write(diary_entry["his"])
            if diary_entry["his_image"]:
                st.image(diary_entry["his_image"], caption="他的图片")
        else:
            st.info("这一天没有日记记录。")
