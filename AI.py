...
def get_ai_response(prompt):
    """AI对话核心"""
    messages = [{"role": "system", "content": SYSTEM_PROMPT}] + st.session_state.messages[-10:] + [{"role": "user", "content": prompt}]
    try:
        res = requests.post(
        ...
        return res.json()["choices"][0]["message"]["content"] if res.status_code == 200 else "我在呢，慢慢说。"
    except:
        return "我在呢，慢慢说。"
...
def apply_theme():
    ...
    /* 聊天容器 */
    .chat-container { margin-top: 120px; height: 60vh; overflow-y: auto; padding: 0 20px; }
    /* 目标删除按钮 */
        ...
        with st.container(border=True, height=100):
            ...
            with col3:
                if st.button("✏️", key=f"cd_edit_{i}", help="确认修改"):
                    ...
                    st.rerun()
            # 显示倒计时结果
            st.metric(label="剩余时间", value=calculate_countdown(st.session_state.countdowns[i]['date']), label_visibility="collapsed")
def render_goals():
...
def render_calendar():
    """日历：点击日期弹出弹窗，弹窗下方显示日程（优化手机端样式）"""
    import streamlit.components.v1 as components  # 用于弹窗组件

    now = get_now()
    year = st.session_state.get("cal_year", now.year)
    month = st.session_state.get("cal_month", now.month)

    # 顶部年月选择（保持简洁）
    col_y, col_m, col_nav = st.columns([0.4, 0.4, 0.2])
    with col_y:
        new_year = st.selectbox(
            "", list(range(2024, 2031)),
            index=year - 2024, key="cal_year", label_visibility="collapsed"
        )
        if new_year != year:
    ...
    for i, name in enumerate(["一", "二", "三", "四", "五", "六", "日"]):
        week_cols[i].markdown(f"<center><small>{name}</small></center>", unsafe_allow_html=True)
    ...
    for week in cal:
        ...
        for idx, day in enumerate(week):
            ...
            is_past = day_date < now.replace(hour=0, minute=0, second=0)
            is_today = day_date.date() == now.date()

            # --- 优化：按钮样式更紧凑，适配手机 ---
            btn_css = f"""
            <style>
            div[data-testid="stButton"][key="cal_btn_{year}_{month}_{day}"] button {{
                border-radius: 50%;
                width: 32px;
                height: 32px;
                font-size: 12px;
                padding: 0;
                {'background-color: #ef4444; color: white;' if is_today else ''}
                {'opacity: 0.5;' if is_past else ''}
            }}
            </style>
            """
            ...
            if col.button(
                ...
                # 存储当前选中日期
                st.session_state.selected_schedule_date = f"{year}-{month:02d}-{day:02d}"
                # 弹出日程编辑弹窗
                with st.container():
                    components.html("""
                    ...
                    st.markdown(f"### 📅 {year}年{month}月{day}日 日程")
                    # 读取已有日程
                    sch_key = f"{year}-{month:02d}-{day:02d}"
                    current_sch = st.session_state.schedule.get(sch_key, "")
                    new_sch = st.text_area(
                        "日程内容", current_sch,
                        key=f"sch_ta_{year}_{month}_{day}",
                        height=80
                    )

                    col_save, col_close = st.columns(2)
                    with col_save:
                        if st.button("✅ 保存", key=f"sch_save_{year}_{month}_{day}"):
                            st.session_state.schedule[sch_key] = new_sch
                            save_all_data()
                            st.success("保存成功！")
                    with col_close:
                        if st.button("❌ 关闭", key=f"sch_close_{year}_{month}_{day}"):
                            components.html("""
...
def main():
    ...
    with st.sidebar:
        ...
        # 聊天背景更换
        st.markdown("**🖼️ 聊天背景**")
        bg_upload = st.file_uploader("", type=["png", "jpg", "jpeg"], key="bg_upload", label_visibility="collapsed")
        if bg_upload:
    ...
    col_ava_user, col_info, col_ava_ai = st.columns([0.1, 0.8, 0.1])
    ...
    with col_ava_user:
        ...
        user_upload = st.file_uploader(
            "", type=["png", "jpg", "jpeg"], key="user_avatar_upload",
        ...
        # 显示头像或默认图标
        if user_img:
            st.image(user_img, width=60, use_container_width=True)
        else:
            st.markdown(
                f"<div style='width:60px;height:60px;background:#e0e0e0;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:24px;'>👤</div>",
                unsafe_allow_html=True)

    # 中间信息
    with col_info:
        st.title(st.session_state.ai_name)
        st.markdown(f"<p>{generate_real_time_status()}</p >", unsafe_allow_html=True)
    ...
    with col_ava_ai:
        ...
        ai_upload = st.file_uploader(
            "", type=["png", "jpg", "jpeg"], key="ai_avatar_upload",
            label_visibility="collapsed", accept_multiple_files=False
        )
        if ai_upload:
            ...
            st.rerun()
        if ai_img:
            st.image(ai_img, width=60, use_container_width=True)
        else:
            st.markdown(
                f"<div style='width:60px;height:60px;background:#e0e0e0;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:24px;'>🤵</div>",
                unsafe_allow_html=True)
