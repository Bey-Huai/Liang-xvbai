    ...
    with st.sidebar:
        ...
        for month in range(1, 13):
            with month_cols[(month - 1) % 4]:
                if st.button(f"{month}月", key=f"month_{month}", type="primary" if st.session_state.selected_month == month else "secondary"):
                    st.session_state.selected_month = month
        with st.container(border=True):
            st.caption(f"📌 {st.session_state.selected_month}月计划")
            st.session_state.year_plan[str(st.session_state.selected_month)] = st.text_area(label="month_plan", value=st.session_state.year_plan[str(st.session_state.selected_month)], height=100, label_visibility="collapsed", placeholder="输入本月核心目标...")
        st.divider()
        ...
        for day in range(1, 8):
            with weekday_cols[day - 1]:
                if st.button(weekdays[day - 1], key=f"day_{day}", type="primary" if st.session_state.selected_weekday == day else "secondary"):
                    st.session_state.selected_weekday = day
        with st.container(border=True):
            st.caption(f"📌 周{weekdays[st.session_state.selected_weekday - 1]}计划")
            st.session_state.week_plan[f"day{st.session_state.selected_weekday}"] = st.text_area(label="week_plan", value=st.session_state.week_plan[f"day{st.session_state.selected_weekday}"], height=100, label_visibility="collapsed", placeholder="输入今日任务...")
        st.divider()
